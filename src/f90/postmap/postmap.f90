...
!    integer(i4b)                          :: ndim, ierr, lmax3    
    ...
    do j = 1, nmaps
       ...
       ! For each masked pixel, look at disc around
       do i = 1, k
          if (ordering==1) then
          ...
          ! If disc contain both masked and unmasked pixels then count it as edge pixel
          if ( any(mask(pixlist(1:numpix),j)==0.d0) .and. any(mask(pixlist(1:numpix),j)==1.d0)) &
               mask(healvec(i),j)=2.d0
          ! if ( count(mask(pixlist(1:numpix),j)==0.d0)>0.3*numpix) mask(healvec(i),j)=2.d0
       end do

       ! Count edge pixels
       k = count(mask(:,j)==2.d0)
       write(*,*) k, '= number of edge pixels'

       ! make mask of edge pixels
       do i = 0, npix-1
    ...
    ! Optionally change values (colors..) in hard code
    ...
    ! clean up
    deallocate(mask, pixlist, healvec)
  ...
  subroutine make_mask(unit)
    ...
    character(len=256)                    :: outprefix, outfile, thickness
    character(len=256)                    :: raname, decname, radiusname, nsidename
    integer(i4b)                          :: i, j, k, pix, numpix
    integer(i4b)                          :: nmaps, nside, npix, ordering
    real(dp)                              :: nullval, vec(3), radius, ra, dec, line, dr
    logical(lgt)                          :: anynull, edge
    ...
    ! Get parameters
    if (iargc() /= 6 .and. iargc() /= 7) then
       write(*,*) 'makemask takes 5(6) parameters'
       call give_user_info
    else 
       call getarg(2, nsidename)
       call getarg(3, raname)         ! in degrees
       call getarg(4, decname)        ! in degrees
       call getarg(5, radiusname)     ! in degrees
       call getarg(6, outprefix)
       if (iargc() == 7) then 
          call getarg(7, thickness)   ! in degrees
          edge = .true.
       else
          edge = .false.
       end if
    end if
    read(nsidename,*) nside
    read(raname,*) ra
    read(decname,*) dec
    read(radiusname,*) radius
    ...
    if (edge) then
       read(thickness,*) dr
       dr = dr*pi/180.d0 ! in radians
       write(*, fmt='(a,f5.2,a)') ' Using line thickness =', dr*180.d0/pi, ' degrees'
    else
       write(*,*) 'Masking the full disc'
    end if
    ! Set map parameters and allocate
    npix  = 12*nside**2
    ordering = 1
    nmaps = 1
    allocate(mask(0:npix-1,nmaps))
    allocate(pixlist(0:npix))
    ! Get disc around given location
    call ang2vec(pi/2-dec, ra, vec)
    write(*,*) radius, '= radius', dr, '= edge'
    if (edge) radius = radius + dr
    write(*,*) radius*180.d0/pi, 'radius'
    if (ordering==1) then
       call query_disc(nside, vec, radius, pixlist, numpix)
    else if (ordering==2) then
       call query_disc(nside, vec, radius, pixlist, numpix, nest=1)
...
subroutine give_user_info
  ...
  if (myid == root) then
     ...
     write(*,*)     
     write(*,*) 'command = makemask: make circular mask of optional thickness (default filled)'
     write(*,*) ' makemask nside ra dec radius outprefix [line thinkness]'
     write(*,*)     
     write(*,*) 'command = edge  : mark edge of mask'    
     write(*,*) ' edge inmask.fits outprefix [line thinkness]'
     write(*,*)     
