...
if __name__ == "__main__":
    ...
    # And set up the acceptlist for patch_6a
...
#### Slightly negotiable
    ...
    acceptlist=secateur.CutScanLength(acceptlist=acceptlist,\
               tmin=tmin,runlist=runlist,patch=patch,verbose=verbose)
    acceptlist.printSummary()
...
#### Negotiable cuts
	...
	#cuttype=7  # 7 10, 30 2-sigma (asymmetric)
        ...
        acceptlist=secateur.CutFailedWeatherDiodes(\
            acceptlist=acceptlist,cuttype=cuttype)
    ...
    if doelcut:
        ...
        acceptlist=secateur.CutElRange(acceptlist,modules,\
                                       elmin,elmax,runlist,verbose,patch)
...
#                        moonthresh=50.0,verbose=verbose)
    ...
    if cuttype == 10:
        print 'Removing bad CESs found by inspecting sub-maps for cuttype %i' \
              % cuttype
